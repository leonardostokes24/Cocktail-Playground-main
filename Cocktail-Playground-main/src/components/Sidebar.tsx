...
const SECTION_META: Record<string, { title: string; id: string }> = {
  spirits:    { title: 'Base Spirits',          id: 'spirits'    },
  liqueurs:   { title: 'Liqueurs',              id: 'liqueurs'   },
  vermouth:   { title: 'Fortified & Vermouth',  id: 'vermouth'   },
  amari:      { title: 'Amari & Aperitifs',     id: 'amari'      },
  citrus:     { title: 'Juices & Citrus',       id: 'citrus'     },
  sweeteners: { title: 'Sweeteners',            id: 'sweeteners' },
  bitters:    { title: 'Bitters & Tinctures',   id: 'bitters'    },
};

const CATEGORY_ORDER = ['spirits','liqueurs','vermouth','amari','citrus','sweeteners','bitters'];

export default function Sidebar({ type, isOpen, user, onTabChange }: {
  type: 'builder' | 'library';
  isOpen: boolean;
  user: any;
  onTabChange: (type: 'builder' | 'library') => void;
}) {
  ...
  useEffect(() => {
    if (!user) { setCustomRecipes([]); return; }
    supabase.from('cocktails').select('*').eq('user_id', user.id).then(({ data, error }) => {
      if (!error && data) setCustomRecipes(data);
    });
  }, [user]);
  ...
  const onDragStart = (event: React.DragEvent, nodeType: string, label: string, categoryType: string, extraData?: any) => {
    event.dataTransfer.setData('application/reactflow', JSON.stringify({ nodeType, label, categoryType, ...extraData }));
    ...
    return (
      <section style={{ marginBottom: '12px' }}>
        <div
          ...
          style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', padding: '8px 0', borderBottom: '1px solid rgba(51,65,85,0.5)' }}
        >
          <h3 className="section-title" style={{ margin: 0 }}>{title}</h3>
          <span style={{ color: '#64748b' }}>{isCollapsed ? <Plus size={12} /> : <Minus size={12} />}</span>
        </div>
  ...
  return (
    <aside className={`sidebar ${type === 'builder' ? 'left' : 'right'} ${!isOpen ? 'collapsed' : ''}`}>
      ...
      <div style={{ marginBottom: '16px' }}>
        <input
          type="text"
          placeholder={type === 'builder' ? 'Search Ingredients...' : 'Search Saved Recipes...'}
          value={search}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', flex: 1, overflowY: 'auto', paddingRight: '4px' }}>
        {type === 'builder' ? (
          <>
            ...
            {loading ? (
              <p style={{ fontSize: '12px', color: '#64748b', textAlign: 'center', padding: '16px 0' }}>Loading ingredients…</p>
            ) : (
              CATEGORY_ORDER.map(cat => {
                const meta = SECTION_META[cat];
                const nodeType = CATEGORY_TO_TYPE[cat] ?? 'modifier';
                const items = (ingredients[cat] ?? []).filter(i =>
                  !search || i.label.toLowerCase().includes(search.toLowerCase())
                );
                if (items.length === 0 && search) return null;
                return (
                  <Section key={cat} title={meta.title} id={meta.id}>
                    {items.length === 0 ? (
                      <p style={{ fontSize: '11px', color: '#64748b', padding: '4px 0' }}>No ingredients yet</p>
                    ) : (
                      items.map(ing => (
                        <div
                          key={ing.id}
                          className="dnd-node"
                          draggable
                          onDragStart={(e) => onDragStart(e, 'ingredient', ing.label, nodeType)}
                        >
                          <span>{ing.label}</span>{' '}
                          <span className={ing.color}>{ing.emoji}</span>
                        </div>
                      ))
                    )}
                  </Section>
      ...
      <div className="mt-8 p-4 bg-slate-900/50 rounded-xl border border-slate-700">
        {type === 'builder' ? (
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', justifyContent: 'center' }}>
              {categories.map(cat => (
                <button
                  ...
                  style={{
                    padding: '6px 10px', borderRadius: '6px', fontSize: '10px', fontWeight: 'bold',
                    cursor: 'pointer', transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)',
                    border: `1px solid ${cat.color}`, color: 'white',
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={() => onTabChange('builder')} style={{ flex: 1, padding: '8px', borderRadius: '8px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', border: 'none', color: 'white', background: '#1e293b' }}>
              🛠️ Switch to Builder
            </button>
            <button onClick={() => onTabChange('library')} style={{ flex: 1, padding: '8px', borderRadius: '8px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', border: 'none', color: 'white', background: '#3b82f6', boxShadow: '0 0 15px rgba(59,130,246,0.4)', outline: '2px solid #60a5fa' }}>
              📚 Library
            </button>
          </div>
        )}
